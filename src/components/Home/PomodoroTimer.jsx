	...
	const PomodoroTimer = () => {
		const { isPlaying, pomodoro, nextPomodoro, playPausePomodoro, sendNotification, updateTimeLeft } = useContext(PomodoroContext)
		const isMobile = useMediaQuery({maxWidth: 767})

		const [isHovering, setIsHovering] = useState(false)
		const [colors, setColors] = useState(pomodoro.colors)
		const [duration, setDuration] = useState(pomodoro.duration)
		const [timeLeft, setTimeLeft] = useState(pomodoro.timeLeft)
		const [key, setKey] = useState(pomodoro.count)
		
		useEffect( () => {
			setKey(pomodoro.count)
			setDuration(pomodoro.duration)
			setTimeLeft(pomodoro.timeLeft)
